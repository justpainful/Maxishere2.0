            ...
            ForEach(targets) { thread in
              Button { forward(to: thread) } label: {
                HStack(spacing: MaxSpace.sm) {
                  ...
                  Text(thread.displayTitle).foregroundStyle(.primary)
                  Spacer()
...
/// The conversation's pinned messages: jump to one, or unpin from its context menu.
struct ChatKitPinnedSheet: View {
  @Environment(MaxAppModel.self) private var model
  @Environment(\.dismiss) private var dismiss
  ...
  private var store: ChatStore { model.chatStore }
  private var state: LoadState<[ChatMessage]>? { store.pinnedMessagesByChat[threadID] }
  private var pinned: [ChatMessage] { state?.value ?? [] }

  var body: some View {
    NavigationStack {
      ScrollView {
        LazyVStack(spacing: 0) {
          if state?.isLoading == true, pinned.isEmpty {
            ProgressView().controlSize(.large).frame(maxWidth: .infinity, minHeight: 160)
          } else if pinned.isEmpty {
            MaxEmptyState(
              title: "No pinned messages",
              subtitle: "Pin a message from its context menu to keep it handy.",
              symbol: "pin"
            )
            .padding(.top, MaxSpace.xl)
          } else {
            ForEach(pinned) { message in
              ...
              .contextMenu {
                Button {
                  Task { _ = await store.setPinned(false, messageID: message.id, in: threadID) }
                } label: {
      ...
      .task { await store.loadPinnedMessages(in: threadID) }
      .maxScreenBackground()
    }
  }
}

/// The promises this conversation is keeping: my pending scheduled messages,
/// each cancellable until the worker sends it.
struct ChatKitScheduledSheet: View {
  @Environment(MaxAppModel.self) private var model
  @Environment(\.dismiss) private var dismiss
  ...
  private var store: ChatStore { model.chatStore }
  private var items: [ScheduledMessage] { store.scheduledByChat[threadID] ?? [] }
  ...
  var body: some View {
    NavigationStack {
        ...
        LazyVStack(spacing: 0) {
          if items.isEmpty {
            MaxEmptyState(
              ...
              subtitle: "Long-press the send button to write now and send later.",
              symbol: "timer"
            )
            .padding(.top, MaxSpace.xl)
          } else {
              ...
              HStack(spacing: MaxSpace.sm) {
                VStack(alignment: .leading, spacing: 2) {
                  Text(verbatim: item.body)
                    ...
                    .lineLimit(2)
                    .frame(maxWidth: .infinity, alignment: .leading)
                  Text(verbatim: sendLabel(item))
                    .font(.caption)
                    .foregroundStyle(MaxColor.accent)
                }
                Button {
                  Task { await store.cancelScheduled(item.id, in: threadID) }
                } label: {
      ...
      .task { await store.loadScheduled(for: threadID) }
      .maxScreenBackground()
    }
  }

  /// "Sends in 45m" while the moment is near, the full date once it is not.
  private func sendLabel(_ item: ScheduledMessage) -> String {
    guard let date = ChatKitTime.date(item.sendAt) else { return "" }
    let seconds = date.timeIntervalSinceNow
    if seconds <= 0 { return "Sending…" }
    if seconds < 60 { return "Sends in \(Int(seconds))s" }
    if seconds < 3600 { return "Sends in \(Int(seconds / 60))m" }
    if seconds < 86_400 { return "Sends in \(Int(seconds / 3600))h" }
    return "Sends \(date.formatted(date: .abbreviated, time: .shortened))"
  }
}

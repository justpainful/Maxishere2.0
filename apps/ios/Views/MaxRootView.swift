...
struct MaxRootView: View {
  ...
  var body: some View {
    ZStack {
      ...
      // The floating mini-player rides above the whole shell (and under the
      // fullScreenCover player, which SwiftUI presents over everything).
      if model.presentsAuthenticatedShell, !model.doubleLockStore.isLocked, !isAppLockGateUp {
        MaxMiniPlayerHost()
      }
    }
    .sheet(item: Binding(
  ...
  /// True while the whole-app Face ID preference holds the shell behind its
  /// gate. The Double Lock check runs first and owns its own screen.
  private var isAppLockGateUp: Bool {
    model.preferencesStore.appLockEnabled && !model.appLockStore.isUnlocked
  }

  private var playerBinding: Binding<MaxMediaItem?> {
    Binding(
      get: { model.doubleLockStore.isLocked || isAppLockGateUp ? nil : model.activePlayer },
  ...
  private var memoriesBinding: Binding<Bool> {
    Binding(
      get: {
        model.isMemoriesEnabled && !model.doubleLockStore.isLocked
          && !isAppLockGateUp && model.isMemoriesPresented
      },

...
extension MaxPlayerSingleView {
  func preparePlayback() async {
    ...
    // A clip armed by the shelf for THIS media becomes the playback bounds.
    // Consuming clears it either way, so a range never outlives one open.
    if let armed = model.consumePendingClip(for: item.id) {
      clipRange = PlayerClipRange(
        clipID: armed.clipID,
        title: armed.title,
        start: armed.startSeconds,
        end: armed.endSeconds
      )
    }
    ...
    if model.isDemoMode {
      ...
      position = min(max(clipRange?.start ?? item.lastPosition, 0), max(duration - 1, 0))
    ...
    do {
      ...
      // A playing clip starts at its range's start, not the resume position.
      position = min(max(clipRange?.start ?? item.lastPosition, 0), max(duration - 1, 0))
      lastPersistedPosition = position
      ...
      attachTimeObserver()
      playbackState = .ready
      // A watch room owns the start: land on the shared target and let the
      // sync loop decide play/pause, so a mid-roll join opens mid-scene.
      if isInWatchRoom, let target = watchStore.computeTarget() {
        position = min(max(target, 0), max(duration - 1, 0))
        await player.seek(
          to: CMTime(seconds: position, preferredTimescale: 600),
          toleranceBefore: .zero,
          toleranceAfter: .zero
  ...
  func togglePlayback() {
    // In a watch room the transport is a shared signal, permission-gated:
    // viewers get the ask-to-control flow instead of a local toggle.
    if isInWatchRoom {
      watchTogglePlayback()
      return
    }
  ...
  func restartPlayback() {
    if isInWatchRoom {
      ...
      watchHardSeek(to: 0)
      Task {
        await watchStore.seek(to: 0, playing: false)
        await watchStore.play(atPosition: 0)
      }
      showControls()
      return
    }
  ...
  func handleProgressTick() {
    // Watch room: the 0.5 s tick doubles as the drift corrector's clock (the
    // overlay runs a twin loop for when the player is paused).
    if isInWatchRoom {
      runWatchSync()
    }
    // Clip bounds: the range's end behaves as the media's end — loop back to
    // its start, or stop with the chrome up.
    if let clipRange, position >= clipRange.end - 0.05 {
      if clipRange.loop {
        seek(to: clipRange.start)
      } else {
        pausePlayback()
        seek(to: clipRange.end)
      }
      return
    }
    ...
    // Re-watch heat: one bin sample every ~6 s of actual playback (the tick
    // fires every 0.5 s). Fire-and-forget — a sample must never block playback.
    if isPlaying {
      heatTickCount += 1
      if heatTickCount >= 12, duration > 0 {
        heatTickCount = 0
        let bin = min(99, max(0, Int((position / duration) * 100)))
        let mediaID = item.id
        let api = model.apiClient
        Task { await api.sampleHeatmap(mediaID: mediaID, bin: bin) }
      }
    }
  ...
  func finishPlayback() {
    ...
    // Watch room: end-of-item advancement is a host duty (loop → queue →
    // shuffle → stop); everyone else waits for the host's media/pause signal.
    // Flow Mode and the Play Next queue never run inside a room.
    if isInWatchRoom {
      Task { await watchStore.handleItemEnded(atPosition: duration) }
      return
    }
    // The Play Next queue outranks Flow Mode: an explicit promise made from a
    // tile's context menu plays immediately, no countdown. Never during a clip.
    if clipRange == nil, model.playQueuedNextIfAny() {
      return
    }
    // Flow Mode: a finished video arms a five-second hand-off to the next one
    // — visible, cancellable, and never during a clip.
    if model.preferencesStore.flowModeAutoAdvance,
       clipRange == nil,
       model.canOpenNextPlayer {
      armUpNext()
    }
  }
  ...
  private func armUpNext() {
    upNextAt = Date().addingTimeInterval(5)
    upNextTask?.cancel()
    upNextTask = Task { @MainActor in
      ...
      guard upNextAt != nil, isFinished else { return }
      upNextAt = nil
  ...
  /// The bookmark button: drops a marker at the current position.
  func dropMarker() {
    let mediaID = item.id
    let at = position
    showControls()
    Task { await model.clipsStore.addMarker(mediaID: mediaID, atSeconds: at) }
  }

  /// Stamps the current position as the draft's A or B point.
  func stampClipPoint(_ which: PlayerClipPoint) {
    switch which {
    case .a: clipDraft.a = position
    case .b: clipDraft.b = position
    }
    showControls()
  }

  /// The scissors button: opens a draft by stamping A, then stamps B.
  func stampNextClipPoint() {
    stampClipPoint(clipDraft.a == nil ? .a : .b)
  }

  func cancelClipDraft() {
    clipDraft = PlayerClipDraft()
    showControls()
  }

  /// Saves the drafted A/B range as a clip. Either stamp order works; an empty
  /// name falls back to the item's title plus the range start.
  func saveClip(named title: String) {
    guard let a = clipDraft.a, let b = clipDraft.b, abs(b - a) >= 1 else { return }
    let start = min(a, b)
    let end = max(a, b)
    let trimmed = title.trimmingCharacters(in: .whitespacesAndNewlines)
    let name = trimmed.isEmpty ? "\(item.displayTitle) · \(start.minuteString)" : trimmed
    let mediaID = item.id
    showControls()
    Task {
      let saved = await model.clipsStore.addClip(
        mediaID: mediaID,
        title: name,
        startSeconds: start,
        endSeconds: end
  ...
  func seekToMarker(_ marker: MediaMarker) {
    seek(to: marker.atSeconds)
    showControls()
  }

  /// Removing a marker needs no confirmation — re-adding is a single tap.
  func deleteMarker(_ marker: MediaMarker) {
    let mediaID = item.id
    showControls()
    Task { await model.clipsStore.removeMarker(mediaID: mediaID, markerID: marker.id) }
  }
  ...
  func deleteCurrent() {
    // The chrome hides trash in a room; belt-and-braces here.
    guard !isInWatchRoom else { return }
    showControls()
    Task {
  ...
  /// Hands playback to the floating mini card: the full player closes and the
  /// card's own AVPlayer resumes at the live position.
  func minimizeToMini() {
    guard isVideo, !model.isDemoMode else { return }
    // Minimize = leave the room (spec §11), confirmed first; the alert's
    // confirm leaves and then re-enters here with the room gone.
    if isInWatchRoom {
      watchUI.showsMinimizeConfirm = true
      return
    }
    let handOffPosition = position
    let wasPlaying = isPlaying
    pausePlayback()
    persistCurrentPosition()
    model.miniPlayerStore.open(item: item, startAt: handOffPosition, playing: wasPlaying)
    onClose()
  }
  ...
  func setPlaybackSpeed(_ speed: Float) {
    // Room rate is a shared, permission-gated signal — never the local
    // preference, and never player.rate directly (the sync loop owns it).
    if isInWatchRoom {
      watchSetSpeed(speed)
      return
    }
  ...
  func showControls(scheduleHide: Bool = true) {
    ...
    // A preference now (2/3/5 s, default 2, matching the desktop): four seconds
    // of chrome over a playing video reads as "stuck", but some people want a
    // beat longer to aim.
    let hideDelay = Double(model.preferencesStore.controlsHideSeconds)

...
final class MaxAppModel {
  ...
  /// The active session's token storage — also what the account switcher
  /// installs a stored account's tokens into.
  let tokenStore: any SessionTokenStore
  /// Every signed-in account's identity + tokens, kept in the keychain.
  let accountStore: MultiAccountStore
  /// The whole-app Face ID gate (armed by the Settings preference).
  ...
  let chatLockStore: ChatLockStore
  ...
  let watchRoomStore: WatchRoomStore
  /// The floating mini card's hand-off state — no dependencies, so it is
  /// simply constructed in place.
  let miniPlayerStore = MiniPlayerStore()
  ...
  /// Items promised to play right after the current video ends — ahead of
  /// Flow Mode's own pick. Small by design; a queue screen would be overkill.
  private(set) var playNextQueue: [MaxMediaItem] = []
  ...
  /// A clip armed by the Clips shelf, consumed by the next player open of its media.
  private var pendingClip: PendingClipRange?
  ...
  init(configuration: MaxConfiguration = .fromBundle()) {
    ...
    // Demo Mode gets its own vault namespace so the switcher never mingles
    // demo rows with real saved accounts.
    self.accountStore = MultiAccountStore(
      service: isDemoMode ? "app.max.iphone.demo" : "app.max.iphone"
    )
    ...
    self.clipsStore = ClipsStore(api: service)
    self.watchRoomStore = WatchRoomStore(api: service)
    self.networkMonitor = NetworkMonitor()
    self.preferencesStore = preferences
    self.doubleLockStore = DoubleLockStore()
    self.chatLockStore = ChatLockStore(defaults: defaults)
  ...
  func handleScenePhase(_ phase: ScenePhase) {
    ...
    case .background:
      ...
      if preferencesStore.appLockEnabled { appLockStore.lockForBackground() }
  ...
  func signOut() async {
    // Signing out forgets the account entirely, so its saved tokens leave the
    // switcher's vault too.
    if let userID = sessionStore.user?.id {
      accountStore.remove(accountID: userID)
    }
    await sessionStore.signOut()
    await clearSessionScopedState()
  }

  /// Resets every session-scoped store and cache — shared by sign-out and the
  /// account-switching paths, which differ only in what happens to the tokens.
  private func clearSessionScopedState() async {
    ...
    ratingStore.reset()
    clipsStore.reset()
    watchRoomStore.reset()
    miniPlayerStore.close()
    playNextQueue = []
    selectedTab = .vault
    activePlayer = nil
    activePlayerQueue = []
  ...
  /// Snapshots the signed-in account (identity + current tokens) into the
  /// switcher's keychain vault. Called after every confirmed session, so the
  /// vault copy tracks token rotation.
  private func recordCurrentAccount() {
    guard !isDemoMode, let user = sessionStore.user,
          let accessToken = try? tokenStore.loadToken(), !accessToken.isEmpty,
          let refreshToken = try? tokenStore.loadRefreshToken(), !refreshToken.isEmpty else {
      return
    }
    accountStore.upsert(
      user: user,
      credentials: SessionCredentials(accessToken: accessToken, refreshToken: refreshToken)
    )
  }

  /// Installs a stored account's tokens and restarts the app state on them.
  /// The outgoing account keeps its session server-side — nothing is revoked,
  /// its latest tokens are simply snapshotted back into the vault.
  func switchAccount(to account: StoredAccount) async {
    guard !isDemoMode, account.id != sessionStore.user?.id else { return }
    recordCurrentAccount()
    await clearSessionScopedState()
    try? tokenStore.saveCredentials(
      SessionCredentials(accessToken: account.accessToken, refreshToken: account.refreshToken)
    )
    await sessionStore.restore()
    // Stale stored tokens clear themselves in restore(); the login screen
    // takes over and the vault row waits for a fresh sign-in.
    guard sessionStore.isAuthenticated else { return }
    recordCurrentAccount()
    await refreshAll()
  }

  /// Stashes the current account and drops the active session locally — no
  /// server-side revoke — landing on the login screen for the new sign-in.
  func beginAddAccount() async {
    guard !isDemoMode, sessionStore.isAuthenticated else { return }
    recordCurrentAccount()
    sessionStore.reset()
    await clearSessionScopedState()
  }
  ...
  /// Queues an item to play right after the current video finishes, ahead of
  /// Flow Mode's own pick. Deduped; a repeated request moves to the back.
  func queuePlayNext(_ item: MaxMediaItem) {
    playNextQueue.removeAll { $0.id == item.id }
    playNextQueue.append(item)
  }

  /// Plays the queue head, if any. Returns true when it consumed one — the
  /// finished player then skips its Flow Mode hand-off.
  @discardableResult
  func playQueuedNextIfAny() -> Bool {
    guard !playNextQueue.isEmpty else { return false }
    let next = playNextQueue.removeFirst()
    // Same no-animation swap as openAdjacentPlayer: replacing the cover's item
    // must not play the whole modal out and back in.
  ...
  /// Arms a saved clip and opens the player on its media. The range applies to
  /// the next player open of that media and only that one, so a stale range can
  /// never attach to a later, plain open (desktop's `openClipNext` pattern).
  func openClip(_ clip: MediaClip) async {
    guard let item = await resolveMediaItem(id: clip.mediaId) else { return }
    pendingClip = PendingClipRange(
      clipID: clip.id,
  ...
  /// The armed clip for this media. Cleared on every call — matching or not —
  /// so it applies to exactly one playback preparation.
  func consumePendingClip(for mediaID: String) -> PendingClipRange? {
    let armed = pendingClip
    pendingClip = nil
    guard let armed, armed.mediaID == mediaID else { return nil }
    return armed
  }

  /// A media item by id: the loaded catalogs first, then a single-item fetch.
  func resolveMediaItem(id: String) async -> MaxMediaItem? {
    if let hit = libraryStore.catalogItems.first(where: { $0.id == id }) {
      return hit
    }
    if let hit = libraryStore.phase.value?.saved.first(where: { $0.id == id }) {
      return hit
    }
    if let hit = homeStore.overview.value?.browse.first(where: { $0.id == id }) {
      return hit
    }
    return try? await apiClient.mediaItem(id: id)
  }

...
final class TransferStore {
  ...
  private func enqueueUpload(_ file: MobileUploadFile, spaceID: String?) -> UUID {
    ...
    let record = TransferRecord(
      ...
      sourceURL: nil,
      // The row could never tell an image from a video from a document — the
      // kind was hardcoded nil even though the MIME type is right here.
      mediaKind: Self.mediaKind(fromMIME: file.mimeType),
      thumbnailURL: nil,
      stagedFileURL: nil,
  ...
  private func enqueueDownload(_ item: MaxMediaItem) -> UUID {
    ...
    let record = TransferRecord(
      ...
      mediaKind: item.kind,
      // The poster was available right here at enqueue time and was discarded,
      // which is why every transfer row showed an arrow instead of the media.
  ...
  /// "image/heic" → "image", "video/mp4" → "video" — the coarse kind the rows
  /// use to pick a glyph. Anything else is a plain file.
  private static func mediaKind(fromMIME mime: String) -> String? {

...
final class LibraryStore {
  ...
  /// Pages the Trash to the end.
  ///
  /// Anything that acts on "everything in the Trash" needs the whole list, not
  /// the page that happens to be on screen - Empty used to purge only what had
  /// been scrolled to while telling the user the Trash had been emptied.
  func loadAllTrash() async {
    if trashPhase.value == nil { await loadTrash() }
    // Bounded so a paging bug cannot spin here forever.
    for _ in 0..<200 {
      guard hasMoreTrash else { return }
      let before = trashedItems.count
      await loadMoreTrash()
      if trashedItems.count == before { return }
    }
  }

  /// Restores several items, reporting how many the server refused.
  func restore(_ items: [TrashedMedia]) async -> Int {
    var failed = 0
    for item in items {
      if await restore(item) == false { failed += 1 }
    }
    return failed
  }

  /// Permanently deletes several items, reporting how many were kept because
  /// they are still shared in a conversation.
  func purge(_ items: [TrashedMedia]) async -> (purged: Int, blocked: Int) {
    var purged = 0
    var blocked = 0
    for item in items {
      switch await purge(item) {
      case .purged: purged += 1
      case .inUse: blocked += 1
      case .failed: break
      }
    }
...
final class ChatStore {
  ...
  /// My pending scheduled messages, per conversation.
  var scheduledByChat: [String: [ScheduledMessage]] = [:]
  /// My saved stickers, favorites first (server-ordered). nil until first load.
  var stickers: [MaxSticker]?
  ...
  /// Debounced server pushes of the local draft, one in flight per thread.
  private var draftSyncTasks: [String: Task<Void, Never>] = [:]
  ...
  func reset() {
    ...
    for task in draftSyncTasks.values { task.cancel() }
    draftSyncTasks = [:]
  }
  ...
  func setDraft(_ draft: String, for threadID: String) {
    ...
    if draft != previous { scheduleDraftSync(draft, for: threadID) }
  ...
  // MARK: - Draft sync
  //
  // Mirrors the desktop's data/chat.ts: local edits push to the server after a
  // 2-second pause, the conversations list carries the synced draft back, and a
  // successful send clears the server copy. All best-effort — a device that
  // cannot sync drafts still chats.

  /// Debounced PUT of the local draft. Each keystroke resets the clock, so the
  /// server only hears the version the user paused on.
  private func scheduleDraftSync(_ draft: String, for threadID: String) {
    draftSyncTasks[threadID]?.cancel()
    draftSyncTasks[threadID] = Task { [weak self] in
      try? await Task.sleep(for: .seconds(2))
      guard !Task.isCancelled else { return }
      await self?.pushDraft(draft, for: threadID)
    }
  }

  private func pushDraft(_ draft: String, for threadID: String) async {
    _ = try? await api.updateChatDraft(chatID: threadID, text: draft)
  }

  /// Installs the server-synced drafts from a freshly loaded conversation list
  /// wherever this device has nothing of its own — a local draft, even an
  /// in-progress empty one the user just cleared, always wins.
  private func installServerDrafts(from chats: [ChatThread]) {
    for chat in chats {
      // `nil`, not "": an entry the user just cleared on this device is a
      // deliberate local state, and a racing list refresh must not undo it.
      guard let server = chat.draft, !server.isEmpty,
            draftsByChat[chat.id] == nil else { continue }
      draftsByChat[chat.id] = server
    }
  }
  ...
  func loadThreads(loadSelectedMessages: Bool = true) async {
    ...
    do {
      ...
      installServerDrafts(from: response.chats)
  ...
  /// Opens (creating on first use) the caller's self-conversation — Saved
  /// Messages — and installs it at the top of the loaded list, exactly the way
  /// a freshly created chat is. Returns the thread so the caller can navigate.
  @discardableResult
  func openSavedMessages() async -> ChatThread? {
    do {
      let response = try await api.savedMessages()
      installCreatedThread(response.chat)
      return response.chat
    } catch {
      createChatError = ProductError.from(error, area: .chats).reason
  ...
  /// Schedules the body to send itself at `sendAt`; the server requires a
  /// moment more than 20 seconds in the future.
  @discardableResult
  func scheduleMessage(in threadID: String, body: String, sendAt: Date) async -> Bool {
    let trimmed = body.trimmingCharacters(in: .whitespacesAndNewlines)
    guard !trimmed.isEmpty else { return false }
    do {
      _ = try await api.scheduleMessage(
        chatID: threadID,
        body: trimmed,
        clientMessageID: UUID().uuidString.lowercased(),
        sendAt: ISO8601DateFormatter().string(from: sendAt)
      )
      await loadScheduled(for: threadID)
      return true
    } catch {
  ...
  func loadScheduled(for threadID: String) async {
    do {
      scheduledByChat[threadID] = try await api.scheduledMessages(chatID: threadID)
    } catch {
      // The sheet simply shows what it last had; nothing to break.
    }
  }

  func cancelScheduled(_ scheduledID: String, in threadID: String) async {
    // Optimistic: the row disappears immediately, truth restored on failure.
    scheduledByChat[threadID] = (scheduledByChat[threadID] ?? [])
      .filter { $0.id != scheduledID }
    do {
      _ = try await api.cancelScheduledMessage(id: scheduledID)
    } catch {
      await loadScheduled(for: threadID)
    }
  }
  ...
  // MARK: - Stickers
  //
  // The tray's state lives here so it survives sheet dismissal: the grid is
  // instant on the second open, and a send started from the tray keeps working
  // after the sheet is gone.

  func loadStickers() async {
    do {
      stickers = try await api.stickers()
    } catch {
      // The tray shows what it last had; nothing to break.
    }
  }

  /// Uploads a new sticker (the caller already downscaled the image), then
  /// reloads so the grid keeps the server's favorites-first ordering.
  @discardableResult
  func createSticker(imageData: Data, mimeType: String, emoji: String? = nil) async -> Bool {
    do {
      let sticker = try await api.createSticker(
        imageData: imageData,
        mimeType: mimeType,
        emoji: emoji
      )
  ...
  /// Optimistic: the tile disappears immediately, truth restored on failure.
  func removeSticker(_ stickerID: String) async {
    stickers = (stickers ?? []).filter { $0.id != stickerID }
    do {
      _ = try await api.deleteSticker(id: stickerID)
    } catch {
      await loadStickers()
    }
  }

  func toggleStickerFavorite(_ stickerID: String) async {
    guard let sticker = (stickers ?? []).first(where: { $0.id == stickerID }) else { return }
    do {
      _ = try await api.favoriteSticker(id: stickerID, favorite: !sticker.favorite)
      await loadStickers()
    } catch {
      // The star simply stays as it was.
    }
  }

  /// Sends a saved sticker; the response carries the full message, so it lands
  /// in the timeline the same way a text send does.
  @discardableResult
  func sendSticker(
    to threadID: String,
    stickerID: String,
    ...
    do {
      let response = try await api.sendSticker(
        chatID: threadID,
        stickerID: stickerID,
        replyToID: replyToID
      )
  ...
  /// Sets or clears the conversation's disappearing-messages timer, then
  /// reloads the list so every surface shows the new setting.
  @discardableResult
  func setMessageTimer(threadID: String, ttlSeconds: Int?) async -> Bool {
    do {
      _ = try await api.setChatTTL(chatID: threadID, ttlSeconds: ttlSeconds)
      await loadThreads(loadSelectedMessages: false)
      return true
    } catch {
      setMutationFailure(error, threadID: threadID)
  ...
  /// Shares the sender's on-device transcription of a voice note, then
  /// refreshes the thread so the transcript renders under the audio bubble.
  @discardableResult
  func submitTranscript(
    ...
    guard !trimmed.isEmpty else { return false }
    do {
      _ = try await api.setVoiceTranscript(
        chatID: threadID,
        messageID: messageID,
        mediaID: mediaID,
        transcript: trimmed
  ...
  func sendMessage(
    ...
    do {
      ...
      // The message left, so the synced draft is spent: cancel any pending
      // debounce and clear the server copy right away.
      draftSyncTasks[threadID]?.cancel()
      Task { [weak self] in await self?.pushDraft("", for: threadID) }
  ...
  /// Sends a recorded voice note through the dedicated inline endpoint, which
  /// stores the audio and posts the message in one round trip.
  @discardableResult
  func sendVoiceMessage(
    ...
    do {
      let response = try await api.sendVoiceMessage(
        chatID: threadID,
        audioData: audioData,
        replyToID: replyToID
      )

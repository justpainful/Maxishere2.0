...
/// GET /api/v2/me/stats — the profile aggregates, computed server-side so the
/// client stops paging the whole catalogue to add four numbers.
/// GET /api/v2/profiles/{id}/stats — what one person may know about another.
/// Every count is scoped server-side to what this viewer can already see, so a
/// private item cannot be revealed by being counted. Deliberately carries no
/// watch history: sharing a vault is not a reason to publish one person's
/// viewing to the other.
struct MaxV2PeerStats: Codable, Sendable {
  let joinedAt: String
  let role: String
  let uploads: Int
...
struct MaxV2ProfileStats: Codable, Sendable {
  let fileCount: Int
  let usedBytes: Int
  let totalViewed: Int
  let totalWatchSeconds: Int
  let ratedFiles: Int
  let ratings: [MaxV2RatedFile]
}

struct MaxV2RatedFile: Codable, Sendable {
  let fileId: String
  let name: String
  let score: Int
...
struct MaxV2Media: Codable, Hashable, Sendable {
  ...
  /// Tokenised adaptive-stream URL once the worker has transcoded this video.
  let hlsUrl: URL?
  let rating: Int?
  let favorite: Bool?
  let isSaved: Bool?
...
struct MaxV2Conversation: Codable, Hashable, Sendable {
  ...
  /// The caller's synced draft, present when one is stored server-side.
  let draft: String?
}
